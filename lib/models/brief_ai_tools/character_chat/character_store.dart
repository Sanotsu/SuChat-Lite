...
class CharacterStore {
  ...
  bool _isCharacterChanged(
    CharacterCard oldCharacter,
    CharacterCard newCharacter,
  ) {
    // 比较关键属性
    return oldCharacter.name != newCharacter.name ||
        ...
        !_areModelsEqual(
          oldCharacter.preferredModel,
          newCharacter.preferredModel,
        );
  }
  ...
  // 导出特定会话的历史记录到用户指定位置
  Future<String> exportSessionHistory(
    String sessionId, {
    String? customPath,
  }) async {
    try {
  ...
  Future<ImportSessionsResult> _importSingleSession(
    ...
    for (var i = 0; i < session.characters.length; i++) {
      ...
      if (!_characters.any((c) => c.id == character.id)) {
        ...
        // 如果角色已存在，使用现有角色
        session.characters[i] = _characters.firstWhere(
          (c) => c.id == character.id,
        );
      }
    }
