...
class BranchManager {
  ...
  List<BranchChatMessage> getMessagesByBranchPath(
    ...
    // 按创建时间排序所有消息
    final sortedMessages = List<BranchChatMessage>.from(allMessages)
    ...
    for (var message in sortedMessages) {
      ...
      if (messagePath.length == pathIndices.length + 1 &&
          messagePath.sublist(0, pathIndices.length).join('/') ==
              pathIndices.join('/')) {
        final parentPath = messagePath
            .sublist(0, messagePath.length - 1)
            .join('/');
        final currentIndex = int.parse(messagePath.last);
        maxChildIndices[parentPath] = max(
          maxChildIndices[parentPath] ?? 0,
          currentIndex,
        );
      }
    }
  ...
  /// 获取同级分支的所有消息
  List<BranchChatMessage> getSiblingBranches(
    List<BranchChatMessage> allMessages,
    ...
    if (message.parent.target == null) {
      return allMessages.where((m) => m.depth == 0).toList()
  ...
  int getBranchIndex(
    ...
    final availableSiblings =
        siblings.where((m) => messages.contains(m)).toList()
          ..sort((a, b) => a.branchIndex.compareTo(b.branchIndex));
  ...
  /// 获取下一个可用的分支索引
  int getNextAvailableBranchIndex(
    List<BranchChatMessage> messages,
    BranchChatMessage message,
    int targetIndex,
  ) {
    ...
    if (targetIndex > getBranchIndex(messages, message)) {
      ...
      final nextMessage = availableSiblings.firstWhere(
  ...
  /// 创建新分支路径
  String createNewBranchPath(BranchChatMessage parentMessage) {
