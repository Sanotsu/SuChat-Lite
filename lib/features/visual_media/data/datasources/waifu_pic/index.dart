...
Future<List<String>> getWaifuPicImages({
  ...
  try {
    if (source == "im") {
      var query = {"included_tags": imIncludedTags, "is_nsfw": imIsNsfw};
      if (imLimit > 1) {
        query.addAll({"limit": imLimit});
      }
      var respData = await HttpUtils.get(
        path: "https://api.waifu.im/search",
      ...
      var respData = (isMany)
          ...
          : await HttpUtils.get(path: "https://api.waifu.pics/$type/$category");
