...
enum MALType { anime, manga, characters, people }
    ...
    children: [
      ...
      Column(
        children: [
          ...
          Text("$total 评价", style: TextStyle(fontSize: 10)),
        ],
      ),
    ],
  );
...
Widget buildBgmScoreArea(double? score, {int? total, int? rank}) {
  return Row(
    ...
    children: [
      Text("${score ?? 0}", style: TextStyle(fontSize: 20)),
      Column(
        children: [
          RatingStars(
            axis: Axis.horizontal,
          ...
          Text("${total ?? 0}人评价", style: TextStyle(fontSize: 12)),
        ],
      ),
      if (rank != null && rank != 0)
        Container(
...
Widget buildFavoritesArea(int? favorites, {int? index}) {
  return Row(
    children: [
      Text("${favorites ?? 0}人最爱", style: TextStyle(fontSize: 12)),
      SizedBox(width: 10),
      index != null
          ? Container(
              width: 90,
      ...
      if (label != null)
        Padding(
          ...
          child: Text(
            label,
            style: TextStyle(
              fontSize: labelFontSize ?? (isSmall == true ? 12 : 14),
            ),
            textAlign: TextAlign.center,
          ),
        ),
      Expanded(
        child: Padding(
          ...
          child: Text(
            value,
            style: TextStyle(
              fontSize: valueFontSize ?? (isSmall == true ? 12 : 14),
...
Widget buildGotoButton(BuildContext context, String label, Widget targetPage) {
...
TextButton buildUrlTitle(BuildContext context, String title, {String? url}) {
  return TextButton(
