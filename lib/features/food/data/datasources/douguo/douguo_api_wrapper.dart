...
import 'douguo_init.dart';

/// 豆果API包装类，提供统一的保护措施
class DouguoApiWrapper {
  static final DouguoApiWrapper _instance = DouguoApiWrapper._internal();
  factory DouguoApiWrapper() => _instance;
  DouguoApiWrapper._internal() {
    // 确保存储已初始化
    _ensureInitialized();
  }

  // 缓存存储
  final GetStorage _cacheStorage = GetStorage(DouGuoInit.douguoCacheKey);

  // 请求记录存储
  final GetStorage _requestLogStorage = GetStorage(
    DouGuoInit.douguoRequestLogKey,
  );
  ...
  // 请求间隔限制
  // 目前所有的豆果都有缓存机制，在缓存时间内重复请求是直接返回的缓存数据，
  ...
  Future<void> _ensureInitialized() async {
    if (!DouGuoInit.isInitialized) {
      await DouGuoInit.init();
    }
  }
  ...
  /// 检查请求频率限制
  /// 实际上目前所有的豆果都有缓存机制，在缓存时间内重复请求是直接返回的缓存数据，
  /// 所以这里请求频率限制requestInterval，可以取消，或者设置很小
...
Future<dynamic> newsGet({
  ...
  return DouguoApiWrapper().request(
    path: path,
