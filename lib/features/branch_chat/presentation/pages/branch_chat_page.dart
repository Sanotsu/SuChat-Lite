...
import 'chat_export_import_page.dart';
...
import '../widgets/branch_message_item.dart';
import '../widgets/branch_tree_dialog.dart';
import '../widgets/branch_message_actions.dart';
import '../widgets/adaptive_chat_layout.dart';
import '../widgets/branch_chat_history_panel.dart';
import '../widgets/adaptive_model_selector.dart';
import '../widgets/draggable_character_avatar_preview.dart';
import '../widgets/text_edit_dialog.dart';
import '../widgets/_small_tool_widgets.dart';
import '../widgets/model_filter.dart';
...
class _BranchChatPageState extends State<BranchChatPage>
  ...
  late MessageFontColor _colorConfig;
  ...
  void initState() {
    ...
    // 获取缓存中的正文文本缩放比例
    textScaleFactor = CusGetStorage().getChatMessageTextScale();

    // 获取缓存的高级选项配置
    if (selectedModel != null) {
      advancedEnabled = CusGetStorage().getAdvancedOptionsEnabled(
        selectedModel!,
      );
      if (advancedEnabled) {
        advancedOptions = CusGetStorage().getAdvancedOptions(selectedModel!);
      }
    }
  ...
  Future<void> showAdvancedOptions() async {
    ...
    if (result != null) {
      ...
      // 保存到缓存
      await CusGetStorage().setAdvancedOptionsEnabled(
        selectedModel!,
        result.enabled,
      );
      await CusGetStorage().setAdvancedOptions(
        selectedModel!,
  ...
  Future<void> showModelSelector() async {
    ...
    advancedEnabled = CusGetStorage().getAdvancedOptionsEnabled(selectedModel!);
    advancedOptions =
        advancedEnabled
            ? CusGetStorage().getAdvancedOptions(selectedModel!)
            : null;
