...
class _CharacterChatPageState extends State<CharacterChatPage>
  ...
  void reloadSession() {
    setState(() {
      currentSession = store.sessions.firstWhere(
  ...
  void showCharacterManagementDialog() {
    ...
    var list = currentSession.characters.map((character) {
      return ListTile(
        ...
        trailing:
            currentSession.characters.length > 1
                ? IconButton(
                  icon: const Icon(
  ...
  Future<void> _processingUserMessage(MessageData messageData) async {
    ...
    try {
      ...
      // 找到消息在列表中的位置
      final index = currentSession.messages.indexWhere(
        (m) => m.id == currentEditingMessage!.id,
      );
      if (index >= 0) {
        // 删除该消息之后的所有回复
